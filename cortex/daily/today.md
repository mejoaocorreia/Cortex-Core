# Today

O dia atual.

## Objetivo
Concentrar tudo o que importa hoje, num só sítio.

## O que vai guardar aqui
- resumo do dia
- foco principal (ver [[cortex/daily/focus|focus]])
- lembretes (ver [[cortex/daily/reminders|reminders]])
- reuniões
- tarefas prioritárias (de [[cortex/qo/priority|qo/priority]])

## Como o CortexCore vai usar
A morning routine prepara este ficheiro de manhã. No fim do dia, o [[cortex/routines/daily_rollover|daily_rollover]] arquiva-o.

## Estado atual
Modelo por preencher. Usar o [[cortex/templates/template_daily|template de daily]].

## Próximos passos
- [ ] definir secções fixas do today
